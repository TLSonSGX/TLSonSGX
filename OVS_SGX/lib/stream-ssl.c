               ...
               enum ssl_state state, struct stream **streamp)
{
     ...
     */
    //printf("@@@@@@@new_ssl_stream state %d\n", state);
//	printf("@@@@@@@new_ssl_stream type %d\n", type);
...
ssl_recv(struct stream *stream, void *buffer, size_t n)
{
    ...
    //printf("--------SSL_read called return is %d\n", ret);
   //printf("---------state after read is %d\n" , SSL_get_state(sslv->ssl));
	...
	//printf("------------ Error after SSL_read is %d\n", error);
...
ssl_do_tx(struct stream *stream)
{
	...
	//printf("---------SSL_write call ret is %d\n", ret);
        if (old_state != SSL_get_state(sslv->ssl)) {
            sslv->rx_want = SSL_NOTHING;
        }
        sslv->tx_want = SSL_NOTHING;
